...
In Backend Example this surfaces as:
...
from utils.db import ScopedSession
from models.config import Config
from utils.auth import get_http_authorization_cred
...
class AuthTokenMiddleware:
    ...
    ``CUSTOM_API_KEY_HEADER`` setting (default ``x-api-key``).
    ...
    set the env var to a unique header (e.g. ``X-API-Key``) so
...
#                     # YouTube URL.
#                     from retrieval.loaders.youtube import _parse_video_id
#
