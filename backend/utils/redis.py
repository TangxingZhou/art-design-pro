...
def get_redis_client(async_mode: bool = False) -> Any | None:
    ...
    """
    sentinel_list = get_sentinels_from_env(settings.REDIS.SENTINEL_HOSTS, settings.REDIS.SENTINEL_PORT)
    if not settings.REDIS.URL and not sentinel_list:
        return None
    try:
        return get_redis_connection(
            settings.REDIS.URL,
            redis_sentinels=sentinel_list,
            redis_cluster=settings.REDIS.CLUSTER,
...
class SentinelRedisProxy:
    ...
    def _should_retry(self, attempt: int) -> bool:
        return attempt < settings.REDIS.SENTINEL_MAX_RETRY_COUNT - 1
    ...
    def _log_retry(self, exc: Exception, attempt: int) -> None:
        log.debug(
            ...
            type(exc).__name__,
            attempt + 1,
            settings.REDIS.SENTINEL_MAX_RETRY_COUNT,
        )
    ...
    def _wrap_async_gen(self, name: str) -> Any:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            async def _inner():
                for attempt in range(settings.REDIS.SENTINEL_MAX_RETRY_COUNT):
                    try:
                        ...
                        if proxy._should_retry(attempt):
                            ...
                            if settings.REDIS.RECONNECT_DELAY:
                                await asyncio.sleep(settings.REDIS.RECONNECT_DELAY / 1000)
                            continue
            ...
            for attempt in range(settings.REDIS.SENTINEL_MAX_RETRY_COUNT):
                try:
                    ...
                    if proxy._should_retry(attempt):
                        proxy._log_retry(exc, attempt)
                        if settings.REDIS.RECONNECT_DELAY:
                            time.sleep(settings.REDIS.RECONNECT_DELAY / 1000)
                        continue
                    proxy._log_exhausted(exc)
                    raise
...
def _socket_options() -> dict[str, Any]:
    ...
    opts: dict[str, Any] = {}
    if settings.REDIS.SOCKET_CONNECT_TIMEOUT is not None:
        opts['socket_connect_timeout'] = settings.REDIS.SOCKET_CONNECT_TIMEOUT
    if settings.REDIS.SOCKET_KEEPALIVE:
        opts['socket_keepalive'] = True
    if settings.REDIS.HEALTH_CHECK_INTERVAL:
        opts['health_check_interval'] = settings.REDIS.HEALTH_CHECK_INTERVAL
    return opts
