...
_url_without_ssl, _ssl_dict = extract_ssl_params_from_url(settings.DATABASE.URL)

# For psycopg2 (sync engine), re-append sslmode + cert-file params.
SQLALCHEMY_DATABASE_URL = reattach_ssl_params_to_url(_url_without_ssl, _ssl_dict) if _ssl_dict else settings.DATABASE.URL
...
_rds_iam_token_auth = RDSIAMTokenAuth(SQLALCHEMY_DATABASE_URL) if settings.DATABASE.ENABLE_IAM_TOKEN_AUTH else None
...
if SQLALCHEMY_DATABASE_URL.startswith('sqlite+sqlcipher://'):
    ...
    if isinstance(settings.DATABASE.POOL_SIZE, int) and settings.DATABASE.POOL_SIZE > 0:
    ...
    def _apply_sqlite_pragmas(dbapi_connection):
        ...
        if settings.DATABASE.ENABLE_SQLITE_WAL:
            cursor.execute('PRAGMA journal_mode=WAL')
        else:
            cursor.execute('PRAGMA journal_mode=DELETE')

        # Each PRAGMA is skipped when its env var is empty, allowing opt-out.
        if settings.DATABASE.SQLITE_PRAGMA_SYNCHRONOUS:
            cursor.execute(f'PRAGMA synchronous={settings.DATABASE.SQLITE_PRAGMA_SYNCHRONOUS}')
        if settings.DATABASE.SQLITE_PRAGMA_BUSY_TIMEOUT:
            cursor.execute(f'PRAGMA busy_timeout={settings.DATABASE.SQLITE_PRAGMA_BUSY_TIMEOUT}')
        if settings.DATABASE.SQLITE_PRAGMA_CACHE_SIZE:
            cursor.execute(f'PRAGMA cache_size={settings.DATABASE.SQLITE_PRAGMA_CACHE_SIZE}')
        if settings.DATABASE.SQLITE_PRAGMA_TEMP_STORE:
            cursor.execute(f'PRAGMA temp_store={settings.DATABASE.SQLITE_PRAGMA_TEMP_STORE}')
        if settings.DATABASE.SQLITE_PRAGMA_MMAP_SIZE:
            cursor.execute(f'PRAGMA mmap_size={settings.DATABASE.SQLITE_PRAGMA_MMAP_SIZE}')
        if settings.DATABASE.SQLITE_PRAGMA_JOURNAL_SIZE_LIMIT:
            cursor.execute(f'PRAGMA journal_size_limit={settings.DATABASE.SQLITE_PRAGMA_JOURNAL_SIZE_LIMIT}')
        cursor.close()
    ...
    if isinstance(settings.DATABASE.POOL_SIZE, int):
...
metadata_obj = MetaData(schema=settings.DATABASE.SCHEMA)
...
if sys.platform == 'win32' and _is_postgres_url(settings.DATABASE.URL):
    import asyncio
...
if 'sqlite' in ASYNC_SQLALCHEMY_DATABASE_URL:
    ...
    _sqlite_pool_size = settings.DATABASE.POOL_SIZE if isinstance(settings.DATABASE.POOL_SIZE, int) and settings.DATABASE.POOL_SIZE > 0 else 512
...
async def get_async_db_context(db: AsyncSession | None = None):
    ...
    if isinstance(db, AsyncSession) and settings.DATABASE.ENABLE_SESSION_SHARING:
        yield db
    else:
