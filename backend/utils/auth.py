...
async def is_valid_token(decoded, redis=None) -> bool:
    ...
    if redis:
        ...
        jti = decoded.get('jti')
        if jti:
            revoked = await redis.get(f'{settings.REDIS.KEY_PREFIX}:auth:token:{jti}:revoked')
            if revoked:
                return False
        ...
        if user_id:
            revoked_at = await redis.get(f'{settings.REDIS.KEY_PREFIX}:auth:user:{user_id}:revoked_at')
            if revoked_at:
                try:
        ...
        if jti and exp:
            ...
            if ttl > 0:
                ...
                await request.app.state.redis.set(
                    f'{settings.REDIS.KEY_PREFIX}:auth:token:{jti}:revoked',
                    '1',
                    ex=ttl,
                )
